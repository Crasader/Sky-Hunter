...
bool Level9::init()
{
	...
	//start game loop
	this->schedule(schedule_selector(Level9::update));

	//start sound loop
	experimental::AudioEngine::play2d("music/Lensko.mp3", true, GameManager::getInstance()->getBgVolume()*0.01);

	//scheduleUpdate();
	schedule(schedule_selector(Level9::update));
	return true;
}

void Level9::initActors()
{
...
void Level9::respawnButtonAction()
{
...
void Level9::awakeEnemyScheduler()
{
	//enemy ratio
	// set up the time delay
	DelayTime *delayAction = DelayTime::create(1.5f);
	// perform the selector call
	CallFunc *callSelectorAction = CallFunc::create(CC_CALLBACK_0(Level9::awakeEnemy, this));
